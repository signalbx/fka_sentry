{{ config(
    ...
    unique_key=['SHIPPING_LABEL_ID', '_3_PL_CUSTOMER', 'ORDER_NUMBER', 'ORDER_ID', 'ORDER_DATE', 'TRACKING_NUMBER'],
    pre_hook=[
        "DELETE FROM {{ source('shiphero_va', 'shipments') }} WHERE ORDER_DATE IS NULL;",
        "DELETE FROM {{ source('shiphero_va', 'shipments_prod') }} WHERE ORDER_DATE IS NULL;",
        "{{ delete_duplicates_shipments('shipments', 'temp_shipments_table') }}",
        "{{ delete_duplicates_shipments('shipments_prod', 'temp_shipments_prod_table') }}",
    ],

    post_hook="TRUNCATE TABLE {{ source('shiphero_va', 'shipments') }};"
...
matched_records AS (
    ...
    JOIN prod
        ...
        AND stage.TRACKING_NUMBER = prod.TRACKING_NUMBER
),
...
non_matched_records AS (
    ...
    WHERE prod.SHIPPING_LABEL_ID IS NULL and prod._3_PL_CUSTOMER IS NULL and 
            prod.ORDER_NUMBER IS NULL and prod.ORDER_ID IS NULL and prod.formatted_date IS NULL and prod.TRACKING_NUMBER IS NULL
),
