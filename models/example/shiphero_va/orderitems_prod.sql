{{ config(
    ...
    pre_hook=[
        ...
        "DELETE FROM {{ source('shiphero_va', 'orderitems_prod') }} WHERE ORDER_DATE IS NULL;",
        "{{ delete_duplicates_orderitems('orderitems', 'temp_orderitems_table') }}",
        "{{ delete_duplicates_orderitems('orderitems_prod', 'temp_orderitems_prod_table') }}",
    ],

    post_hook="TRUNCATE TABLE {{ source('shiphero_va', 'orderitems') }};"
...
prod AS (
    SELECT
        ...
        PROFILE_ID,
        _FIVETRAN_SYNCED
    FROM {{ source('shiphero_va', 'orderitems_prod') }}
    WHERE formatted_date is not null
),
