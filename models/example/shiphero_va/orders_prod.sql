{{ config(
    ...
    pre_hook=[
        ...
        "DELETE FROM {{ source('shiphero_va', 'orders_prod') }} WHERE ORDER_DATE IS NULL;",
        "{{ delete_duplicates_orders('orders', 'temp_orders_table') }}",
        "{{ delete_duplicates_orders('orders_prod', 'temp_orders_prod_table') }}",
    ],

    post_hook="TRUNCATE TABLE {{ source('shiphero_va', 'orders') }};"
...
prod AS (
    SELECT
        ...
        ORDER_DATE_UTC,
        _FIVETRAN_SYNCED
    FROM {{ source('shiphero_va', 'orders_prod') }}
    WHERE formatted_date is not null
),
